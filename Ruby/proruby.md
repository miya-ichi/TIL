...
- 2.7.3は問題なくインストールできた。irbでも日本語入力可能に。
## 第2章
### 改行について
...
### setupメソッド
Minitestではsetupメソッドを定義すると、テストコードの実行前に毎回実行してくれる
## 第4章
### ブロック
...
クラス名.メソッド名 or クラス名::メソッド名→クラスメソッドを表す
### 定数
大文字と数字とアンダースコアで名前をつける
### 継承
継承元のクラスをスーパークラスまたは親クラス  
継承先のクラスをサブクラスまたは子クラスと呼ぶ  
クラスの継承関係を表す際にはクラス図を使用(親クラスに矢印の先が向く)  
継承を使う場合は機能ではなく性質や概念の共通点に着目する(is aの関係になっているか)  
Rubyの標準ライブラリは全てBasicObjectクラス-Objectクラスを継承している  
独自に作成したクラスはデフォルトでObjectクラスを継承する  
他のクラスを継承したい場合は`class 〇〇 < スーパークラス`で定義  
スーパークラスのメソッドを呼び出す際はsuperを使用する（同名メソッドを呼び出す）  
スーパークラスとサブクラスで処理が同じ場合は、サブクラスで定義しなくても自動的にスーパークラスのメソッドが実行される  
継承を利用する場合はスーパークラスの実装を把握しておくことが望ましい  
- 意図せずメソッドや変数名をサブクラスで上書きしてしまい、バグが起こる可能性
### privateメソッドについて
private記述後に定義したメソッドは、クラス外では呼び出せない（正確にはレシーバを付けた呼び出しができない）  
スーパークラスで定義したprivateメソッドは、サブクラス内で呼び出すことは可能・オーバーライドも可能  
クラスメソッドはprivate後に記述しただけではクラス外で呼び出すことができてしまう  
privateなクラスメソッドを定義したい場合は、`class << self`構文にて定義する  
`private`は実はメソッドであり、引数としてprivateにしたいメソッド名を渡すことで、後から変更することができる  
### protectedメソッドについて
同じクラスまたはサブクラスの中からであればレシーバ付きで呼び出せる。  
外部に公開したくないが、同じクラスのインスタンスからは参照したい場合などに使用する
### 定数について
クラスの外部から参照可能で、参照する場合は`クラス名::定数名`とする
定数とはいうものの、数値は後から変更が可能！(原則、変更はしない)  
ミュータブルな値を扱っている場合は、プログラム内での破壊的変更に気をつける  
- .freezeで破壊的変更を防ぐことができるが、配列やハッシュの場合は各要素ごとに行う必要もある
### ローカル変数とインスタンス変数以外の変数
使用頻度は少ない
- クラスインスタンス変数:@変数名（クラス定義直下で宣言する）
- クラス変数:@@変数名
- グローバル変数:$変数名
- Ruby標準の組み込み変数(特殊変数)
### 演算子の再定義
演算子も(全部ではないが)メソッドであり、クラスの定義内で再定義することが可能  
### オープンクラス
Rubyのクラスは定義済みのものに追加・上書きができる（オープン）なため、オープンクラスと呼ばれることもある  
Railsは積極的にオープンクラスを活用している  
### モンキーパッチ
既存のメソッドを上書きして自分の期待する挙動に変更すること  
外部ライブラリに不具合や要件に合わない挙動があったときにモンキーパッチをあてて挙動を変更することがある  
オープンクラスと共に、意図せず不具合を招かないよう使用には注意を払う  
### 特異メソッド
特定のオブジェクトに対して定義したメソッド  
`def オブジェクト.メソッド名`で定義できる  
定義したオブジェクトにのみ使用ができる  
数値・シンボルには特異メソッドを定義できない  
クラスメソッドも実は特異メソッドである  
